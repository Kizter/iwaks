...
- **Amendemen M3 (slice 8 — navigasi library):** sidebar Songs/Albums/Artists/Folders menjadi view nyata (`view` state di `App.tsx`); Albums/Artists/Folders **diturunkan frontend-side** dari `getTracks()` (`groupBy` — grup album/artist/folder, kartu grid memakai cover track pertama, urutan alphabet case-insensitive); klik kartu → daftar lagu grup (play dari posisi grup, filter search tetap berlaku), tombol back + judul breadcrumb (`back-btn`); link Playlists **ditunda** (keputusan user) dan dihapus dari nav. `refreshKey` juga mem-perbarui grid setelah scan/add.
  ...
  - **Amendemen M4 (slice 3 — folder browse):** view struktur folder **hierarkis**, diturunkan **frontend-side** dari `showing` — tanpa command backend baru (pola sama dengan grid Albums/Artists, Amendemen M3 slice 8). Helper pure TS di `App.tsx`: `parentOf` (parent dir, separator-agnostic; drive/path root → `null`), `folderSegments` (segmen path root→leaf + path penuh tiap segmen, untuk breadcrumb), `buildFolderTree` (`Map<dir → {direct, subfolder, cover}>` + `roots` = dir yang parent-nya **tidak** punya track sendiri; `subfolder` = jumlah child dir langsung yang berisi track; `cover` = track pertama untuk sampul kartu), `folderNameCompare` (sort case-insensitive per display name). Grid **Folders** kini menampilkan **root saja** — pengganti grid datar M3 slice 8 yang menampilkan semua distinct dir sebagai saudara (parent & child tercampur). Detail folder: **breadcrumb** di toolbar (nav `<ol>`; tiap segmen = tombol drill-up ke dir tersebut, segmen terakhir = folder aktif non-interaktif; `<h1 class="visually-hidden">` tetap untuk heading) + back button lama tetap; section **Subfolders** (kartu `group-card`: cover, nama, `<n> songs · <m> subfolders`) untuk drill-down; lalu **track langsung** folder tsb (model Poweramp: play = file di folder ini saja — `onPlay` atas `groupTracks` yang sudah ada). Empty state folder: tanpa search & tanpa track langsung tapi ada subfolder → *"No tracks directly in this folder — music lives in the subfolders below."*. Search mem-filter pohon (tree dibangun dari `showing`) — konsisten dengan grid browse lain; header `track-head` (Title/Time) disembunyikan saat folder punya subfolder agar tidak tampil di atas kartu. Murni frontend (App.tsx + App.css) — tidak ada kode Rust/command baru.
...
| M4 | Playlist + queue + folder browse + tag editor | Slice 1 (playlist + m3u import/export) ✅, Slice 2 (queue: view + drag-reorder + save-as-playlist) ✅, Slice 3 (folder browse: root grid + drill-down + breadcrumb) ✅ — tag editor menyusul |
